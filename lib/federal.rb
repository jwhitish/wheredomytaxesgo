...
def fed_tax_bracket_joint(gross_income)
  case gross_income
  when gross_income < 19050
    #10% of taxable income
    flat_tax = 0
    tax_rate = .1
  when gross_income >= 19051 && gross_income < 77400
    #flat tax of $1905 plus 12% of amount over $19050
    tax_rate = .12
    flat_tax = 1905
  when gross_income >= 77401 && gross_income < 165000
    #flat tax of $8907 plus 22% of amount over $77400
    flat_tax = 8907
    tax_rate = .22
  when gross_income >= 165001 && gross_income < 315000
    #flat tax of $28179 plus 24% of amount over $165000
    flat_tax = 28179
    tax_rate = .24
  when gross_income >= 315001 && gross_income < 400000
    #flat tax of $64179 plus 32% of amount over $315000
    flat_tax = 64179
    tax_rate = .32
  when gross_income >= 400001 && gross_income < 600000
    #flat tax of $91379 plus 35% of amount over $400000
    flat_tax = 91379
    tax_rate = .35
  when gross_income >= 600001
